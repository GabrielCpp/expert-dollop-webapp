...
export function TranslationScope({ name, ressourceId, children }: TranslationScopeProps) {
    const { reduxDb, axios, routes } = useServices<Services>();
    const { i18n } = useTranslation()
    const [cursor, setCursor] = useState<AsyncRestCursor<Translation> | undefined>(new AsyncRestCursor<Translation>(
        axios, 
        500, 
        (nextPageToken: string | undefined, limit: number) => routes.render(name, { ressourceId, locale: i18n.language }, {...(nextPageToken && {nextPageToken}), limit})
    ));
