...
export type OnTableChange = (insertions: TableRecord[], deletions: TableRecord[], updates: [TableRecord, TableRecord][]) => void;

export class TableChangeEmitter implements TableWatcher {
    private eventEmitter = new EventEmitter();
    
    public triggerOnCommit(insertions: TableRecord[], deletions: TableRecord[], updates: [TableRecord, TableRecord][]): void {
        this.eventEmitter.emit(ON_TABLE_COMMIT, insertions, deletions, updates);
    }

    public addOnCommitSubscriber(callback: OnTableChange): Unsubcribe {
