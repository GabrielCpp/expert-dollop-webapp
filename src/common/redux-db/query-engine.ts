...
export type RecordPredicate = (record: TableRecord, otherRecord?: TableRecord) => boolean
...
function buildPredicate(parameters: QueryParameter, filter: FilterNode | []): RecordPredicate {
    function evalTree(record: TableRecord, otherRecord: TableRecord , filter: FilterNode): unknown {
        ...
        if(prefix === '$') {
            const args = children.map(child => evalTree(record, otherRecord, child))
    ...
    return (record: TableRecord, otherRecord: TableRecord = {}): boolean => Boolean(evaluate(record, otherRecord, filter as FilterNode))
}

function buildSortCompareFn(sorts: QuerySort[], isLessThan: (a: unknown, b: unknown) => boolean): SortCompareFn {
...
export class QueryExecutor {
    ...
    public addLayer(tableName: string, where: FilterNode | []): void {
        ...
        this.unsubscribe = table.tableEventEmitter.addOnCommitSubscriber((insertions: TableRecord[], deletions: TableRecord[], updates: [TableRecord, TableRecord][]) => {
            const hasInsertion = insertions.some(record => predicate(record))
            const hasDeletion = deletions.some(record => layer.resultset.has(buildPk(record)));
            const hasUpdate = updates.some(([_, record]) => layer.resultset.has(buildPk(record)));
    ...
    public execute(): TableRecord[] {
        ...
        const wherePredicate = buildPredicate(this.query.parameters, this.query.where);
        ...
        for(const queryJoin of this.query.joins) {
            ...
            const joinPredicate = buildPredicate(this.query.parameters, queryJoin.where);
            ...
            for(const queryRecord of records) {
                for(const candidateRecord of targetTable.records) {
                    if(joinPredicate(queryRecord, candidateRecord)) {
                        combinedRecords.push({ ...queryRecord, [queryJoin.toProperty]: candidateRecord})
                    }
                }
            }
        }
        ...
        if(this.query.projections.length > 0) {
            postProcessedRecords = postProcessedRecords.map(record => this.query.projections.reduce((previousValue, [path, alias]) => {
